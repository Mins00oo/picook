...
│   ├── recipes/
│   │   ├── RecipeList.tsx                 # 목록 (코칭준비 ✅/❌, 상태 뱃지)
│   │   ├── RecipeForm.tsx                 # 등록/수정 (active/wait + duration)
...
│   └── recipe/
│       ├── RecipeStepEditor.tsx            # 조리 순서 편집 (active/wait + duration)
│       ├── IngredientMapper.tsx            # 재료 매핑 (자동완성 + 수량/단위)
│       └── CoachingReadyIndicator.tsx      # coaching_ready 상태 표시
├── hooks/
...
- 조리 순서: 번호 + 설명(텍스트에리어) + 이미지(선택) + **step_type(active/wait 라디오)** + **duration_seconds(초, 필수)** + **can_parallel(체크박스)**
- 순서 변경: 드래그앤드롭
- duration 미입력 → coaching_ready = false 경고 + 저장은 가능
- 분류 가이드: 폼 상단에 상시 표시 "능동 = 썰기, 볶기, 섞기 / 대기 = 끓이기, 익히기, 밥 짓기"
...
└──────────┴──────────┴──────────┴──────────┘
┌──────────┬──────────┬──────────┬──────────┐
│ 총 레시피 │ 공개중   │ 코칭 이용 │ 쇼츠 변환 │
│   500    │  420     │  42%     │  234건   │
└──────────┴──────────┴──────────┴──────────┘

[차트: 일별 가입자/검색/코칭/쇼츠 추이 — 기간 필터 7d/30d/90d]
